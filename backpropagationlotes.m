...
v1=v1+etha*(deltak'*entradas(:,1))/size(entradas,1);
v2_h=v2_h+etha*sum(grad_Ep)/size(entradas,1);
v3=v3+etha*deltak'*entradas(:,2)/size(entradas,1);
v4=v4+etha*sum(deltak)*Bias/size(entradas,1);
%Actualización de los pesos w
w1=w1+etha*Delta_j'*entradas(:,1)/size(entradas,1);
w2=w2+etha*Delta_j'*entradas(:,2)/size(entradas,1);
w3=w3+etha*sum(Delta_j)*Bias/size(entradas,1);
epocas=epocas+1;
cont2=cont2+1;
